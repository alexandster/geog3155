---
title: "GEOG 3155/6155 Lab 02 Spatial data in R Part 2"
knit: (function(input_file, encoding) {
  out_dir <- 'docs';
  rmarkdown::render(input_file,
 encoding=encoding,
 output_file=file.path(dirname(input_file), out_dir, 'GEOG_3155_6155_lab02_part2.html'))})
author: | 
  | Alexander Hohl
...
ca_places <- st_read("./datafiles/ca_places/ca_places.shp")
...
b5 <- rast("./datafiles/rs/LC08_044034_20170614_B5.tif")
...
We can write `rast` objects back to file using `writeRaster()` (I'll bet you never thought it would be called that). You can write out to any format supported by [GDAL](https://gdal.org/drivers/raster/index.html). Here we write out to a TIFF format. You can see the full list of available formats for reading and writing by running the `writeFormats()` function in the console. We'll use this again after having worked with the data.
...
The function `global()` can be used to calculate most summary statistics for a raster layer. So to get the mean global temperature (and standard deviation):
...
```{r}
# Blue
b2 <- rast('datafiles/rs/LC08_044034_20170614_B2.tif')
# Green
b3 <- rast('datafiles/rs/LC08_044034_20170614_B3.tif')
# Red
b4 <- rast('datafiles/rs/LC08_044034_20170614_B4.tif')
# Near Infrared (NIR)
b5 <- rast('datafiles/rs/LC08_044034_20170614_B5.tif')
```
...
filenames <- paste0('./datafiles/rs/LC08_044034_20170614_B', 1:11, ".tif")
...
tracts <- st_read("./datafiles/slc_tract/slc_tract_2020.shp")
## Salt Lake light rail tracks
lightrail <- st_read("./datafiles/LightRail_UTA/LightRail_UTA.shp")
## Salt Lake light rail stations
stations <- st_read("./datafiles/LightRailStations_UTA/LightRailStations_UTA.shp")
...
tracts_sub = tracts %>%
  dplyr::filter(TRACTCE20 %in% c(102500, 102600, 114000))
...
tm_shape(tracts) + 
  tm_fill("density", title = "Popn density", palette = "Greens", style = "quantile", id = "TRACTCE20") +
  tm_borders("lightgray") +
  tm_shape(lightrail) +
  tm_lines(lwd = 2) +
  tm_shape(stations) +
...
For this lab, you need to make a map.

- Undergraduates use the tmap package to make a static map of total housing units per census tract in Salt Lake County (HU100 column of tracts dataset).

- Graduates use leaflet to make an interactive map of [trailheads](https://gis.utah.gov/products/sgid/recreation/trailheads/) in Utah.

You should submit the following through Canvas by September 8th:
