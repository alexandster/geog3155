...
Next load the libraries you will need for the lab. You should at this stage have most of these already installed. Add anything that is not installed using the `install.packages()` function. Note that the `vip` package needs to be installed from archive as follows:

```{r message=FALSE}
url <- "https://cran.r-project.org/src/contrib/Archive/vip/vip_0.4.6.tar.gz"
pkgFile <- "vip_0.4.6.tar.gz"
...
Students enrolled in 6155 should also provide
