...
To plot these, you can use **tmap**'s `tm_fill` function, where you'll need to specify the name of the layer you want to map. So to map the `log10_carea` values:

```{r}
tm_shape(env) +
  tm_fill(fill = "log10_carea") +
  tm_shape(lsl_sf) +
  tm_dots()
```

This function can be used with a second function (`tm_scale_interval`) which allows you to change the color palette and number of breaks (this uses a red palette and quantile breaks):
